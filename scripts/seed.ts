...
async function seed() { for (const name of ["patients", "visits", "prescriptions", "pharmacyOrders", "queueEvents", "notifications", "referralRequests", "doctors", "medicines", "queues"]) await clearCollection(name); for (const staff of STAFF) await ensureStaff(staff); const db = adminDb(); const batch = db.batch(); doctors.forEach(({ id, ...doctor }) => batch.set(db.collection("doctors").doc(id), { ...doctor, hospitalId:"H1", currentVisitId:null, updatedAt:FieldValue.serverTimestamp() })); medicines.forEach(([id, name, unit, unitPrice]) => batch.set(db.collection("medicines").doc(id), { name, unit, unitPrice, stockQuantity:100, lowStockThreshold:10, stockStatus:"available", active:true, updatedAt:FieldValue.serverTimestamp() })); await batch.commit(); console.log("Clinicify reset: 2 doctors, named staff, and medicine catalogue only. No patient, visit, prescription, referral, bill, or queue demo data exists."); }
